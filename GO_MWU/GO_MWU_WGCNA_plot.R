...
# change from go.obo to goslim_generic.obo if necessary
goDivision= args[2] # either MF, or BP, or CC
source("gomwu.functions.R")

# Plotting results
levs <- as.numeric(c(args[5], args[6], args[7]))
h <- as.numeric(args[8])*2.5 # 8
w <- as.numeric(args[8])*6.5 # 11
pdf(file = paste("../../outputs/WGCNA-results/GO_MWU/figures/", args[2], "_", args[3], sep = ""), height = h, width = w)#, units = "in", res = 150)
results = gomwuPlot(input, goAnnotations, goDivision,
	...
	txtsize = as.numeric(args[8])*1.5,  # decrease to fit more on one page, or increase (after rescaling the plot so the tree fits the text) for better "word cloud" effect
	treeHeight = 1.0, # height of the hierarchical clustering tree
	colors = rep(args[4],4) # these are default colors, un-remar and change if needed
)
dev.off()
...
#return(results)
setwd("/Users/mikeconnelly/computing/projects/EAPSI_Pocillopora_AxH")
