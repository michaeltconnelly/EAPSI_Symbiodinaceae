...
```{r}
for (i in 1:10) {
  genoboxplot(DE$sig[[5]]$Gene.model[i])
}
```
...
dpvarc <- as.data.frame(dp1$var.contr) %>%
  rownames_to_column("Gene.model") %>% left_join(gene_annotation, by = "Gene.model") %>%
  arrange(desc(LD1))
```
```{r dapc_loadingplot}
set.seed(4)
...
# create heatmap
pdf("../outputs/charts/DAPC_LD1_10genes_heatmap_clustered.pdf", height = 8.5, width = 11)
...
```
```{r dapc_boxplots_ld1}
#pdf("../outputs/charts/DAPC_LD1_loadings_10genes_boxplots.pdf", height = 3.5, width = 5)
for (i in 1:10) {
  genoboxplot(dpvarc_ld1$Gene.model[i])
}
```
```{r dapc_heatmaps_ld2}
...
#pdf("../outputs/charts/DAPC_LD2_10genes_heatmap_clustered.pdf", height = 8.5, width = 11)
...
for (i in 1:10) {
  genoboxplot(dpvarc_ld2$Gene.model[i])
}
```
...
## Weighted gene co-expression network analysis (WGCNA) (DONE)
...
#modkME <- genecolors_kME %>% 
modkME <- select(genecolors_kME, "gene", "moduleColor", paste("kME", module, sep = ""))
View(modkME)
