---
title: "Symbiodinaceae_AXH_transcriptome_analysis_october"
...
# knitr::opts_knit$set(root.dir = "~/Documents/Research/EAPSI_Symbiodinaceae/")#Rmd/
knitr::opts_knit$set(root.dir = "~/computing/projects/EAPSI_Pocillopora_AxH/")
options(stringsAsFactors = FALSE)
```
## (DONE) Setup packages and working directories
...
## (DONE) Import sample metadata, counts data, and gene annotation
```{r data, echo=TRUE}
...
countdata <- read.csv("../outputs/AxH_SymC1.csv")
...
#View(countdata)
# Merge Geneid column and counts columns, cutting out middle columns
#countcols <- (countdata[ ,7:ncol(countdata)])
#idcol <- (countdata[1])
#combinedcols <- data.frame(idcol, countcols)
# Clip counts data so same number of rows as annotation data (there are more counts Geneid's than annotation ID's)
#clipcounts <- merge(combinedcols, gene_annotation, by.x = "Geneid", by.y = "ID", all.x = FALSE, all.y = TRUE)
# Showing unmatched counts genes
#preclipcounts <- merge(combinedcols, gene_annotation, by.x = "Geneid", by.y = "ID", all.x = TRUE)
#sum(is.na(preclipcounts$Description))
#Adding missing IDs to dataframe
#Missing = NULL
#for(i in 1:length(preclipcounts$Description)) { 
  #if(is.na(preclipcounts$Description[i])) {
    #print(preclipcounts$Geneid[i])
    #Identifiers = preclipcounts$Geneid[i]
    #Missing = rbind(Missing, data.frame(Identifiers))
  #}
#}
#write.csv(Missing, "~/Desktop/EAPSI_Pocillopora_AxH/Missing.csv")

# Remove UniProt and Description columns
#clipcounts <- clipcounts[1:48]
# Set Gene ID's as row names
#row.names(clipcounts) <- clipcounts$Geneid
# Sort countdata by P. damicornis gene ID
#countdata.sorted <- as.data.frame(clipcounts[order(rownames(clipcounts)),])
#countdata.sorted <- countdata.sorted[-1]

# Mike's original data...
# Set Gene ID's as row names
#--row.names(countdata) <- countdata$Geneid
...
```
```{r gene data}
#gene_annotation <- read.delim(file = "./data/genome_annotations.tab", header = T) %>% arrange(ID)

#gene_annotation <- read.csv("~/Desktop/SymbC1.Gene_Models.Annotation.csv")
gene_annotation <- read.csv("./data/genome/SymbC1.Gene_Models.Annotation.csv")
# gene_annotation <- read.delim(file = "./data/pdam_genome_annotations.tab", header = T) %>% arrange(ID)
rownames(gene_annotation) <- gene_annotation$Gene.model
gene_annotation <- as.data.frame(gene_annotation[order(rownames(gene_annotation)),])


# Check gene feature annotation and countdata rowname order coherence
all(rownames(countdata.sorted) == gene_annotation$Gene.model)
all(rownames(countdata.sorted) ==  rownames(gene_annotation))

# Obtain KOG annotations for P. damicornis genome
gene2kog <- gene_annotation %>% select(Gene.model, KOG_Class) %>% filter(KOG_Class != "")
...
treatment_annotation <- HeatmapAnnotation(Treatment = c("control", "Heat", "Antibiotics", "Antibiotics.Heat"),
                                          col = anno_colors[2])
```
## (DONE) Create DESeqDataSet
```{r create_dds}
# Create full DESeqDataSet
countData = countdata.sorted
colData = coldata
...
## (DONE) Visualize global gene expression
```{r vst}
# Normalize expression data for visualization purposes using VST tranformation
vsd <- vst(dds, blind = TRUE)
```
```{r}
PCA <- ggPCA(vsd, samples, condcolors_AxH, pclab = c(1,2))
PCA
ggsave(PCA, filename = "../outputs/charts/PCA.png", width = 170, height = 100, units = "mm", device = "png")
```
...
pcoa1
ggsave(pcoa1, filename = "../outputs/charts/pcoa_colony.png", width = 150, height = 150, units = "mm", device = "png")
```
```{r treatment pcoa}
...
pcoa2
ggsave(pcoa2, filename = "../outputs/charts/pcoa_treatment.png", width = 112, height = 80, units = "mm", device = "png")
```
...
ggsave(varplot, filename = "../outputs/charts/FigS6_variancePartition_total.pdf", width = 120, height = 85, units = "mm", device = "pdf")
```
```{r varpart_sum}
...
## (DONE) Differential gene expression analysis (DESeq2)
```{r genotypic DESeq}
...
```{r DEG table}
DE <- DE %>%
  mutate(df = map(dsr, ~ rownames_to_column(data.frame(.), "Gene.model")),
         df = map(df, ~ left_join(., gene_annotation, by = "Gene.model")))
...
# Select top 10 DEGs for all sig controlled antibiotics 

allanti <- read_csv("../outputs/DESeq-results/tables/sig/Antibiotics_control_all_sig.csv")
sortedanti <- allanti[order(allanti$padj),]
top10anti <- sortedanti[1:10,]
top10antiwithdescriptions <- sortedanti[1:12,]
View(top10antiwithdescriptions)

# Select top 10 DEGs for all sig controlled antibiotics.heat 

allanti.heat <- read_csv("../outputs/DESeq-results/tables/sig/Antibiotics.Heat_control_all_sig.csv")
sortedanti.heat <- allanti.heat[order(allanti.heat$padj),]
top10anti.heat <- sortedanti.heat[1:10,]
View(top10anti.heat)

# Select top 10 DEGs for all sig controlled heat 

allheat <- read_csv("../outputs/DESeq-results/tables/sig/Heat_control_all_sig.csv")
sortedheat <- allheat[order(allheat$padj),]
top10heat <- sortedheat[1:10,]
View(top10heat)
...
shared_DEGs_colonies <- DE %>%
  ...
  filter(Colony != "all") %>%
  group_by(num, den) %>%
  dplyr::count(Gene.model, log2FoldChange > 0) %>%   # In how many colonies is same gene DE in same direction?
...
shared_DEGs_treatments <- DE %>%
  ...
  filter(den == "control") %>% 
  group_by(Colony) %>%
  dplyr::count(Gene.model, log2FoldChange > 0) %>% #View()
...
#--ressig_heat <- DE$sig[DE$num == "Heat" & DE$den == "control" & DE$Colony == "all"][[1]]$Gene.model
#--ressig_anti <-  DE$sig[DE$num == "Antibiotics" & DE$den == "control" & DE$Colony == "all"][[1]]$Gene.model
#--ressig_anti.heat <-  DE$sig[DE$num == "Antibiotics.Heat" & DE$den == "control" & DE$Colony == "all"][[1]]$Gene.model

ressig_heat <- DE$sig[DE$num == "Heat" & DE$den == "control" & DE$Colony == "all"][[1]]
ressig_anti <-  DE$sig[DE$num == "Antibiotics" & DE$den == "control" & DE$Colony == "all"][[1]]
ressig_anti.heat <-  DE$sig[DE$num == "Antibiotics.Heat" & DE$den == "control" & DE$Colony == "all"][[1]]
```
```{r}
#--AH <- intersect(ressig_anti, ressig_heat)
AH <- merge(ressig_anti, ressig_heat, by.x = "Gene.model", by.y = "Gene.model", all.x = FALSE, all.y = FALSE)
#View(AH)
#--AX <- intersect(ressig_anti, ressig_anti.heat)
AX <- merge(ressig_anti, ressig_anti.heat, by.x = "Gene.model", by.y = "Gene.model", all.x = FALSE, all.y = FALSE)
#View(AX)
#--HX <- intersect(ressig_heat, ressig_anti.heat)
HX <- merge(ressig_heat, ressig_anti.heat, by.x = "Gene.model", by.y = "Gene.model", all.x = FALSE, all.y = FALSE)
#View(HX)
#--AXH <- intersect(intersect(ressig_anti, ressig_heat), ressig_anti.heat)
AXH <- merge(AH, ressig_anti.heat, by.x = "Gene.model", by.y = "Gene.model", all.x = FALSE, all.y = FALSE)
#View(AXH)
...
```
```{r treatment_venn}
pdf(file = "../outputs/DESeq-results/figures/Venn_AXH.pdf", height = 4, width = 4) # Not working...
...
## (DONE) DESeq results visualizations
### Volcano plots
...
ggsave(filename = "../outputs/charts/all_anti.control_volcano.pdf", height = 3, width = 4, device = "pdf")
```
...
commandArgs <- function(...) c("MWU_BP_heat.control.logP.txt", "BP")
#source("../R/GO_MWU/GO_MWU.R")
source(knitr::purl("../R/GO_MWU/GO_MWU.R", quiet=TRUE))

# Run GO_MWU for Heat vs. control for Molecular Function GO terms
commandArgs <- function(...) c("MWU_MF_heat.control.logP.txt", "MF")
#source("../R/GO_MWU/GO_MWU.R")
source(knitr::purl("../R/GO_MWU/GO_MWU.R", quiet=TRUE))

# Run GO_MWU for Heat vs. control for Cellular Component (CC) GO terms
commandArgs <- function(...) c("MWU_CC_heat.control.logP.txt", "CC")
#source("../R/GO_MWU/GO_MWU.R")
source(knitr::purl("../R/GO_MWU/GO_MWU.R", quiet=TRUE))
```
```{r heat.control_GO}

x <- as.list(c("../R/GO_MWU/MWU/"))
...
anti.control.GO %>%
  select(ontology, delta.rank, p.adj, name, nseqs, level, term) %>%
  arrange(ontology, p.adj) %>% 
  write_csv(path = "../R/GO_MWU/tables/anti.control.GO.csv")
...
View(kogtables$Heat.control)
KOG_heat.control <- KOGheatmap(kogtables$Heat.control, kogpvals$Heat.control)
...
kog_contrasts <- bind_cols(kogtable_contrast, kogpval_contrast)
...
KOG_treatments <- KOGheatmap(
  select(kogtable_contrast, Heat.control, Antibiotics.control, Antibiotics.Heat.control),
  select( kogpval_contrast, Heat.control, Antibiotics.control, Antibiotics.Heat.control),
...
ggsave(filename = "../outputs/charts/Fig2_DAPC.png", plot = dapcplot, width = 120, height = 115, units = "mm", device = "png")
```
...
# create heatmap
pdf("~/Desktop/DAPC_LD1_10genes_heatmap_clustered.pdf", height = 9, width = 11)
...
```
```{r dapc_boxplots_ld1}
pdf("~/Desktop/DAPC_LD1_loadings_10genes_boxplots.pdf", height = 3.5, width = 5)
for (i in 1:10) {
...
pdf("~/Desktop/DAPC_LD2_10genes_heatmap_clustered.pdf", height = 9, width = 11)
...
```
```{r dapc_boxplots_ld2}
#pdf("../outputs/charts/DAPC_LD2_loadings_10genes_boxplots.pdf", height = 3.5, width = 5)
for (i in 1:10) {
...
## (DONE) Weighted gene co-expression network analysis (WGCNA)
...
net <- blockwiseModules(
  ...
  saveTOMs = TRUE, 
  saveTOMFileBase = "~/Desktop",
  # saveTOMFileBase = "./outputs/WGCNA-results/AxH_TOM",
  # Basic tree cut options
  deepSplit = 2, # 2
  detectCutHeight = 0.99, # 0.99
  minModuleSize = min(50, ncol(datExpr)/5 ), # 50 minimum, One-fifth of all genes in expression matrix (4487) maximum
  # Module merging options
  mergeCutHeight = 0.25, # 0.3
...
minModuleSize <- min(25, ncol(datExpr)/5 )
...
```{r trait_data}
# Get sample data into tibble
samplesmeta <- read.table("../data/EAPSI_samples_AxH_meta.txt", header = TRUE)
datTraits <- samplesmeta %>% 
...
#-------------------------- Receiving "Error: Can't subset columns that don't exist. x Column `kMEblack` doesn't exist."
...
#modkME <- genecolors_kME %>% select(., "gene", "moduleColor", paste("kME", module, sep = ""))
  modkME <- genecolors_kME %>% select(., "gene", "moduleColor", paste(module, sep = ""))
}
...
commandArgs
source("./GO_MWU_WGCNA_plot.R")
# previously Mike's: source("./R/GO_MWU/GO_MWU_WGCNA_plot.R")
#}
```
```{r modules_GO}
modules.GO <- as.list(
...
GS <- as.numeric(cor(datExpr, datTraits$Antibiotics.Heat, use = "p"))
  ...
  numberOfColumn <- substring(colnames(datKME[2:22]), 4) # datkME not found
```
```{r}
 for (module in uniqModules) {
...
g <- hikME %>% ggplot(., aes(GS, kME)) + geom_point(color = module) + 
  ...
  ylim(-0,1.0) +
  theme_bw() +
  geom_text_repel(data = hikME[1:10, ], aes(label = hikME[1:10, ]$Description.s..of.hit.s.), color = "black", size = 2.5, box.padding = unit(0.35, "lines"))
print (g)
}
dev.off()
```
