---
title: "Cladocopium_AXH_transcriptome_analysis_october_mike"
...
## Import sample metadata, symbiont counts data, and symbiont gene annotation
```{r, echo=TRUE}
...
write_csv(shared_DEGs_colonies, path = "./outputs/DESeq-results/tables/shared_DEGs_colonies.csv")
```
...
write_csv(shared_DEGs_treatments, path = "./outputs/DESeq-results/tables/shared_DEGs_treatments.csv")
```
...
```
```{r treatent_eulerr}
axh_list <- c(6, 771, 3436, 0, 4, 1154, 2)
...
```
```{r}
axh_list_up <- c(2, 442, 1771, 0, 1, 499, 2)
...
```
```{r}
axh_list_down <- c(4, 330, 1666, 0, 3, 654, 0)
...
# Retaining 10 PC's gives highest a-score (30%) and utilizes 70.5% of the cumulative variance. Retaining 11 PC's gives a high a-score (29.8%) and uses 72.4% of cumulative variance. Therefore, let's use 11 PC's for the DA. 
```
```{r}
...
dapcplot <- plot_grid(xplot, NULL, dapc.fig, yplot,
                      ...
                      greedy = FALSE)
dapcplot
# ggsave(filename = "./outputs/DAPC-results/figures/DAPC.pdf", plot = dapcplot, width = 120, height = 115, units = "mm", device = "pdf")
...
dpvarc <- as.data.frame(dp1$var.contr) %>%
  ...
  arrange(desc(LD1))
write_csv(dpvarc, path = "./outputs/DAPC-results/tables/DAPC_loadings_table.csv")
...
dpvarc %>% 
  ...
  head(20) %>% 
  select(-LD2) %>% 
  write_excel_csv(path = "./outputs/DAPC-results/tables/LD1_table2.csv")
```
...
```
```{r dapc_boxplots_ld1}
pdf("./outputs/DAPC-results/figures/DAPC_LD1_loadings_10genes_boxplots.pdf", height = 3.5, width = 5)
for (i in 1:20) {
...
pdf("./outputs/DAPC-results/figures/DAPC_LD_loadings_scatterplot.pdf", height = 7, width = 7)
dpvarc %>% ggplot(., aes(x = LD1, y = LD2)) +
  geom_point() +
  xlim(c(0,0.0025)) +
  ylim(c(0,0.0025)) +
  geom_text_repel(data = dpvarc_ld2[1:10, ], aes(label = (dpvarc_ld2[1:10, 6])), color = "black", size = 2.5, box.padding = unit(0.35, "lines")) + ggtitle("DAPC LD gene loadings scatterplot", subtitle = "Top 10 genes LD2 labelled")
```
...
datTaxa_Family <- read_csv("./data/datTaxa_Family.csv") %>% column_to_rownames("SampleID")
...
col_fun_KOGfisher <- colorRamp2(c(15, 0), c("red", "white"))
