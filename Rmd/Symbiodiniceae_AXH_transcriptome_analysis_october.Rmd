...
```
## Setup packages and working directories (DONE)
...
library("extrafont")
library("extrafontdb")
# extended visualization functions for symbiont analysis
source("./R/AXH_functions_C1.R")
```
```{r colors}
...
countdata <- read.csv("./outputs/SymC1_counts/AxH_SymC1.csv", comment.char="#")
=======
...
>>>>>>> 99d76c7a5eebc2d038fe8a608224fc431c1dd5cd
## Tidy counts data
...
```{r gene data}
# Need to redo genome annotation to incorporate GO and KOG annotations from egg-NOG-mapper
...
gene_annotation <- read.csv("./data/SymbC1.Gene_Models.Annotation.csv", header = T) %>%
...
```
## Begin gene expression analyses that correspond to the host analysis
...
# Need to adjust for Symbiont gene sets
...
# heat.control.GO %>% count(ontology)

class(heat.control.GO)


heat.control.GO$ontology
...
# https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/Tutorials/FemaleLiver-02-networkConstr-man.pdf - reference step 2b for info on choosing the soft-thresholding power
...
pdf(file = "./outputs/WGCNA-results/cladocopium_dendrogram.pdf", width = 8, height = 6)
# pdf(file = "./manuscript_figures/FigS##_WGCNA_cladocopium_dendrogram.pdf", width = 8, height = 6)
...
pdf(file = "./outputs/WGCNA-results/ME_complexheatmap_treatments_cladocopium.pdf", width = 6.5, height = 6)
